...
package uk.co.exemel.disco.disco.netutil.nio.marshalling;
...
import uk.co.exemel.disco.disco.marshalling.impl.to.ComplexTO;
import uk.co.exemel.disco.disco.marshalling.impl.to.EnumTO;
import uk.co.exemel.disco.disco.marshalling.impl.to.EnumType;
import uk.co.exemel.disco.disco.marshalling.impl.to.TO;
import uk.co.exemel.disco.disco.marshalling.impl.util.ByteArrayClassLoader;
import uk.co.exemel.disco.disco.marshalling.impl.util.EnumCreator;
import uk.co.exemel.disco.disco.marshalling.impl.util.Pair;
...
import uk.co.exemel.disco.disco.marshalling.impl.util.ComplexObjectCreator;
...
public class SocketRMIMarshallerArgsTest {
	...
	public void testAdditionalField() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, SecurityException, NoSuchFieldException {
		...
		ComplexObjectCreator complexObjectCreator = new ComplexObjectCreator("TO");
	...
	public void testEnumAdded() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, SecurityException, NoSuchFieldException {
		...
		EnumCreator enumCreator = new EnumCreator("EnumType" ,new String[] {"ONE","TWO"});
		enumCreator.create(bos);

		ByteArrayClassLoader twoEnum = new ByteArrayClassLoader(EnumType.class.getName(), bos.toByteArray());
		Class<Enum> twoEnumClass = (Class<Enum>) Class.forName("EnumType",true,twoEnum);
		Enum TWO = Enum.valueOf(twoEnumClass, "TWO");
		bos.reset();

		ComplexObjectCreator complexObjectCreator = new ComplexObjectCreator("EnumTO");
		complexObjectCreator.create(bos, new Pair[]{ new Pair<String,String>("EnumType","enumType")});

		ByteArrayClassLoader bcl = new ByteArrayClassLoader(twoEnum,"EnumTO", bos.toByteArray());
		Class<?> enumTOClass = Class.forName("EnumTO", true, bcl);
	...
	public void testEnumRemoved() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException, SecurityException, NoSuchFieldException {
		...
		EnumCreator enumCreator = new EnumCreator("EnumType" ,new String[] {"ONE","TWO","THREE","FOUR"});
		enumCreator.create(bos);

		ByteArrayClassLoader enumCL = new ByteArrayClassLoader("EnumType",bos.toByteArray());
		Class<Enum> enumClass = (Class<Enum>) Class.forName("EnumType",true,enumCL);
		Enum FOUR = Enum.valueOf(enumClass, "FOUR");
		bos.reset();

		ComplexObjectCreator complex = new ComplexObjectCreator("EnumTO");
		complex.create(bos, new Pair[] {new Pair<String,String>("EnumType","enumType")});

		ByteArrayClassLoader bcl = new ByteArrayClassLoader(enumCL, "EnumTO", bos.toByteArray());

		Class<?> clazz = Class.forName("EnumTO", true, bcl);
		Object o = clazz.newInstance();
